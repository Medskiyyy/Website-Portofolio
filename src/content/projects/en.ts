...
export const projectCopyEn: Record<string, ProjectCopy> = {
  "pempek-cek-lis": {
    ...
    lessonsLearned: [
      "Row-Level Security is worth the setup cost — the access rule lives next to the data instead of being re-implemented in every query.",
  ...
  "hitung-uang": {
    ...
    overview:
      "A native Android personal finance app with no backend at all. Transactions live in Room on the device, the app locks behind a PIN or biometrics, receipts are scanned with on-device OCR, spending charts are drawn with Compose Canvas, and backups export to a ZIP file the user controls. Not published to the Play Store — the source is on GitHub, and I am still building on it.",
    problem:
